...
NeoHook also ships a C ABI (`include/neohook.h`, generated by cbindgen) and a
header-only C++ wrapper on top of it (`include/neohook.hpp`). These examples are
what keeps both honest: they are the only place a real C/C++ compiler ever
consumes those headers, and CI builds and runs them for x64 *and* x86 on every
push.

Each one is self-verifying — it checks what it prints and exits non-zero if a
hook does not behave as documented.

| Example | What it shows |
| :------ | :------------ |
| [`c/inline_hook.c`](c/inline_hook.c) | The transaction API: `attach_export`, the trampoline, disable/enable, unhook. |
| [`c/iat_hook.c`](c/iat_hook.c) | IAT hook, and why an import slot must be re-read rather than cached. |
| [`c/introspect.c`](c/introspect.c) | Module/export/import enumeration, name and ordinal lookup, signature scanning. |
| [`c/watchdog.c`](c/watchdog.c) | Anti-tamper watchdog with a tamper callback on the background thread. |
| [`c/abi_conformance.c`](c/abi_conformance.c) | Compile-time checks that the header's `HookContext` layout and callback conventions match the stub. |
| [`cpp/transaction.cpp`](cpp/transaction.cpp) | `neohook::Transaction` / `HookGuard` RAII, including rollback on an abandoned transaction. |
| [`cpp/vtable_hook.cpp`](cpp/vtable_hook.cpp) | COM-style vtable hooking, shared table and per-instance clone. |
| [`cpp/midhook.cpp`](cpp/midhook.cpp) | Mid-function detour rewriting a register through `HookContext` (x64) . |
| [`cpp/breakpoint_hooks.cpp`](cpp/breakpoint_hooks.cpp) | `VehHook` and `Int3Hook`, both with a gateway to the original. |
| [`cpp/watchdog.cpp`](cpp/watchdog.cpp) | The watchdog through the `neohook::Watchdog` guard. |

### Building

`build.ps1` builds the cdylib with cargo, compiles every example with MSVC
(`/W4 /WX`) against the generated import library, copies `neohook.dll` next to
the executables, and optionally runs them:

```powershell
# from an MSVC developer prompt
.\examples\build.ps1 -Run

# 32-bit, and without a developer prompt
.\examples\build.ps1 -Target i686-pc-windows-msvc -Run `
    -VcVarsAll "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvarsall.bat"
```

Artifacts land in `target/examples-native/<target>/`, prefixed `c_` or `cpp_`.

To consume the headers from your own project, add `include/` to the include path
and link against `neohook.dll.lib` (or the `neohook.lib` static library) from
