...
impl TrampolineAlloc {
    ...
    pub unsafe fn alloc_nearby(target: *const u8, size: usize) -> Option<*mut u8> {
        ...
        {
            unsafe { Self::alloc_nearby_x64(target, size) }
        }
    }

    /// Allocates a region of memory that is within 2GB of the target address, which is necessary for x86 relative jumps. This function is specifically optimized for x64 architecture.
    #[cfg(target_arch = "x86_64")]
    unsafe fn alloc_nearby_x64(target: *const u8, size: usize) -> Option<*mut u8> {
